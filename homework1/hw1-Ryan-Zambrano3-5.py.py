...
# Assuming `softmax_weight` and `softmax_bias` have been correctly reshaped and initialized as per your code

def compute_input_gradient(asample, softmax_weight, softmax_bias, y_true):
    logits = np.dot(asample, softmax_weight) + softmax_bias
    probabilities = softmax(logits)
    gradient_logits = probabilities - y_true
    gradient_wrt_input = np.dot(softmax_weight, gradient_logits)
    return gradient_wrt_input, probabilities
...
epsilon = 0.01  # Perturbation size for the L-infinity norm constraint
max_iterations = 100
tolerance = 1e-6  # Tolerance for minimal change detection

for i in range(max_iterations):
    gradient_wrt_input, probabilities = compute_input_gradient(asample, softmax_weight, softmax_bias, y_true)
    predicted_class = np.argmax(probabilities)

    # Break if the correct class is achieved
    if predicted_class == correct_label:
        print("Target class achieved.")
        break

    # Update the sample with controlled perturbation
    perturbation = np.sign(gradient_wrt_input) * epsilon
    asample -= perturbation  # Adjust the sample in the opposite direction of the gradient

    # Ensure we're making meaningful updates
    if np.linalg.norm(perturbation, np.inf) < tolerance:
        print("Minimal changes; stopping iteration.")
        break
